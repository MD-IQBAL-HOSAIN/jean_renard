@extends('backend.layout.app', ['title' => 'Edit Album'])
...
@section('main')
    <div class="container p-3 border border-black"  style="width: 50vw; border-radius: 10px">
        <h1 class="text-center text-3xl font-bold mb-6">Edit Album Page</h1>
        <hr class="mb-6">
        ...
        <form action="{{ route('album.update', $album->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                @if ($album->image)
                    <img src="{{ asset('storage/' . $album->image) }}" alt="Current Image" class="mt-2 w-24 h-auto">
                @endif
                @error('image')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="mb-4">
                ...
                <select id="user_id" name="user_id"
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ $user->id == old('user_id', $album->user_id) ? 'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </select>
